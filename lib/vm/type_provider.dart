...
class TypeNotifier extends AsyncNotifier<List<Types>> {
  ...
  Future<List<Types>> fetchTypes() async {
    var url = Uri.parse("${GlobalData.url}/type/select"); // 본인 서버 주소
  ...
  Future<void> addType(Types t) async {
    ...
    state = await AsyncValue.guard(() async {
      var url = Uri.parse("${GlobalData.url}/insert");
  ...
  Future<void> updateType(Types t) async {
    state = await AsyncValue.guard(() async {
      var url = Uri.parse("${GlobalData.url}/update");
      await http.post(url, body: t.toJson());
  ...
  Future<void> deleteType(int id) async {
    state = await AsyncValue.guard(() async {
      var url = Uri.parse("${GlobalData.url}/delete?id=$id");
      await http.get(url);
      return fetchTypes();
    });
  }
